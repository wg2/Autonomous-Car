...
def rrt(rm, start, goal, bias=0.05, eta=0.5, max_iter=1000):
    ...
    for _ in range(max_iter):
        ...
        # BEGIN QUESTION 5
        rand_sample = sample(rm, goal_config, bias)[0]
        nearest_id, nearest_vertex = tree.GetNearestVertex(rand_sample)
        new_sample = extend(nearest_vertex, rand_sample, eta)
        # END QUESTION 5
        ...
        # BEGIN QUESTION 5
        if rm.problem.check_state_validity(new_sample):
            if rm.problem.check_edge_validity(nearest_vertex[0], new_sample[0]):
                new_id = tree.AddVertex(new_sample)
                tree.AddEdge(nearest_id, new_id)

                if rm.problem.goal_criterion(new_sample, goal_config):
                    x_goal_id = new_id
                    break
        # END QUESTION 5
