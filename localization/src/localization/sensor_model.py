...
class SingleBeamSensorModel:
    ...
    def precompute_sensor_model(self, max_r):
        ...
        #z_hit
        if self.hit_std > 0:
            prob_z_hit = np.exp(-0.5 * (diff**2) / (self.hit_std**2)) / (self.hit_std * np.sqrt(2 * np.pi))
        else:
            prob_z_hit = (diff == 0).astype(float)
        #z_short
        lambda_short = 1.0
        prob_z_short = np.where(obs_r < sim_r, np.exp(-lambda_short * obs_r), 0.0)
        #z_max
        prob_max = (obs_r == max_r).astype(float)
        #z_rand
        if max_r > 0:
            prob_rand = np.where(obs_r < max_r, 1.0 / max_r, 0.0)
        else:
            prob_rand = 0.0
        #table
        prob_table = (self.z_hit * prob_z_hit + self.z_short * prob_z_short + self.z_max * prob_max + self.z_rand * prob_rand)
        col_sums = np.sum(prob_table, axis=0, keepdims=True)
        prob_table = np.divide(prob_table, col_sums, out=np.full_like(prob_table, np.nan), where=col_sums > 0)
        # END QUESTION 2.1
