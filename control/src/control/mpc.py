...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        controls[:, :, 1] = np.linspace(min_delta, max_delta, self.K)[:, np.newaxis]
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        for t in range(self.T):
            rollouts[:, t + 1, :] = (
                rollouts[:, t, :]
                + self.motion_model.compute_changes(rollouts[:, t, :], controls[:, t, :], dt)
            )
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        final_states = rollouts[:, -1, :2]  # K x 2
        diffs = final_states - reference_xyt[:2]  # K x 2
        distances = np.linalg.norm(diffs, axis=1)  # K
        return distances * self.error_w
        # END QUESTION 4.3
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        collisions = self.check_collisions_in_map(rollouts.reshape(-1, 3)).reshape(self.K, self.T + 1)
        coll_cost = collisions.sum(axis=1) * self.collision_w
        return coll_cost
        # END QUESTION 4.3
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        return self.sampled_controls[best_idx, 0, :]
        # END QUESTION 4.4
